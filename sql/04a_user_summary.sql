...
SELECT
    ...
    ROUND(
        1.0 * b.got_invite / NULLIF(b.job_applies, 0), 3
    )                                                               AS recruiter_response_rate
...
SELECT
    ...
    , 1) AS wow_growth_pct,
    ROUND(
        AVG(weekly_active_applicants) OVER (
            ORDER BY event_week
...
DROP TABLE IF EXISTS aarrr_summary;

CREATE TABLE aarrr_summary AS
SELECT 'Acquisition' AS metric,
    COUNT(DISTINCT user_id) AS value,
    'Total users who started a session' AS description
FROM user_summary

UNION ALL SELECT 'Activation',
    SUM(is_activated),
...
UNION ALL SELECT 'Referral',
    COUNT(DISTINCT user_id),
    'Users acquired via referral channel'
FROM user_summary
WHERE source = 'referral'

UNION ALL SELECT 'Revenue',
    ROUND(SUM(estimated_ltv_usd), 0),
    'Estimated platform LTV (USD proxy)'
FROM ltv_proxy;

DROP TABLE IF EXISTS platform_health_score;

CREATE TABLE platform_health_score AS
SELECT
    ROUND(
        (activation_rate_pct * 0.35) +
        ((100 - ghosting_rate_pct) * 0.30) +
        ((100 - churn_rate_pct) * 0.25) +
        (avg_relevance_score * 100 * 0.10)
    , 1) AS health_score,
    activation_rate_pct,
    ghosting_rate_pct,
    churn_rate_pct,
